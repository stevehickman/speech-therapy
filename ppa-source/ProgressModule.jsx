...
export const DEFAULT_PROGRESS_SETTINGS = {
  ...
  retentionDays: 90,
  moduleDetail: {
    // "therapist" defaults to "none": verbatim therapy conversation is PHI and
    // should only be included in reports after an explicit caregiver opt-in.
    therapist: "none", naming: "detailed", assessment: "detailed",
...
export const RETENTION_OPTIONS = [
  ...
  // "Keep all" (value 0) is intentionally omitted from the standard picker.
  // It is available after an explicit confirmation — see the admin view below.
];

// One-time migration: strip verbatim chat message content from any progress log
// entries written before the privacy fix (TherapistModule used to log the full
// message text as `content`).  Now only the event marker is stored.
// Safe to run repeatedly — no-ops on already-clean entries.
export function ppaStripChatContent() {
  try {
    ...
    for (const key of progressKeys) {
      try {
        ...
        const cleaned = entries.map(e => {
          if (e.type !== "chat" || !Object.prototype.hasOwnProperty.call(e, "content")) return e;
          dirty = true;
          // Keep only the safe fields; drop `content` (verbatim patient words)
          return { type: "chat", result: e.result ?? "sent", time: e.time };
        });
        if (dirty) localStorage.setItem(key, JSON.stringify(cleaned));
      } catch {}
    }
  } catch {}
}

// Delete progress entries older than the configured retention window.
// Called once on app startup so stale data is removed automatically.
export function ppaRunRetentionPurge() {
  try {
    const raw = localStorage.getItem(PROGRESS_SETTINGS_KEY);
    const settings = raw ? { ...DEFAULT_PROGRESS_SETTINGS, ...JSON.parse(raw) } : DEFAULT_PROGRESS_SETTINGS;
    const retentionDays = settings.retentionDays ?? DEFAULT_PROGRESS_SETTINGS.retentionDays;
    ...
    Object.keys(localStorage)
      .filter(k => /^ppa_progress_\d{4}-\d{2}-\d{2}$/.test(k))
      .filter(k => k.slice("ppa_progress_".length) < cutoffStr)
...
export default function ProgressModule({ sessionLog, bkt, bktSnapshots, conditionType }) {
  ...
  const emailReport = () => {
    // PHI must not appear in the mailto: URL — URLs land in browser history and
    // the Referer header, and are visible to anyone who uses the device.
    // The caregiver uses the Copy button to get the full report, then pastes it.
    const addr = settings.emailAddress || "";
    const subj = encodeURIComponent(`PPA Progress Report ${reportRange.start} to ${reportRange.end}`);
    const body = encodeURIComponent("Please paste the copied report here.");
    ...
    return (
      <div style={{ padding: 20, maxWidth: 680, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ ...card }}>
          ...
          <div style={{ marginTop: 10, padding: "10px 13px", background: "#FFF8E8", border: "1px solid #F0E0A0", borderRadius: 10, fontSize: 13, color: "#7A5010", lineHeight: 1.6 }}>
            <strong>Privacy:</strong> Reports are generated by Anthropic's AI — practice activity data is sent to generate them.
            {(settings.moduleDetail?.therapist || "none") !== "none" && (
              <span> AI Therapist is set to <strong>{settings.moduleDetail.therapist}</strong> — therapy conversation excerpts will be included. You can change this in <button onClick={() => setView("admin")} style={{ background: "none", border: "none", color: "#4E8B80", cursor: "pointer", fontWeight: 700, fontSize: 13, padding: 0, textDecoration: "underline" }}>Settings</button>.</span>
            )}
          </div>
        </div>
              ...
              <>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
                  ...
                  <button onClick={emailReport} style={{ ...btn("#E8F4F2", "#2D5A54"), padding: "8px 16px" }}
                    title="Opens your email client — copy the report first, then paste it into the email">
                    ✉️ {settings.emailAddress ? `Email to ${settings.emailAddress}` : "Email (copy first)"}
                  </button>
  ...
  if (view === "admin") {
    ...
    return (
      <div style={{ padding: 20, maxWidth: 680, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ ...card }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#2D3B36", marginBottom: 8 }}>🗓️ Progress Data Retention</div>
          <div style={{ fontSize: 13, color: "#666", marginBottom: 12, lineHeight: 1.5 }}>
            Session records older than this are deleted automatically each time the app opens.
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {RETENTION_OPTIONS.map(o => (
              <button key={o.value} onClick={() => setES(prev => ({ ...prev, retentionDays: o.value }))}
                style={{ padding: "7px 14px", borderRadius: 20, border: "2px solid", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
                  borderColor: (es.retentionDays ?? 90) === o.value ? "#4E8B80" : "#D5CFC4",
                  background: (es.retentionDays ?? 90) === o.value ? "#E8F4F2" : "#FFFDF9",
                  color: (es.retentionDays ?? 90) === o.value ? "#2D5A54" : "#666" }}>
                {o.label}
              </button>
            ))}
            {/* "Keep all" requires explicit confirmation — therapy records can build
                a detailed multi-year clinical history with no automatic cleanup. */}
            {(es.retentionDays ?? 90) === 0 ? (
              <button onClick={() => setES(prev => ({ ...prev, retentionDays: 90 }))}
                style={{ padding: "7px 14px", borderRadius: 20, border: "2px solid #4E8B80", fontSize: 13,
                  fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
                  background: "#E8F4F2", color: "#2D5A54" }}>
                Keep all ✓
              </button>
            ) : (
              <details style={{ display: "inline-block" }}>
                <summary style={{ padding: "7px 14px", borderRadius: 20, border: "2px solid #D5CFC4",
                  fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
                  background: "#FFFDF9", color: "#888", listStyle: "none" }}>
                  Keep all…
                </summary>
                <div style={{ position: "absolute", zIndex: 10, background: "#FFFDF9", border: "2px solid #F0A0A0",
                  borderRadius: 12, padding: "12px 16px", marginTop: 4, maxWidth: 320, boxShadow: "0 4px 16px rgba(0,0,0,0.15)" }}>
                  <div style={{ fontSize: 13, color: "#8B2D2D", marginBottom: 10, lineHeight: 1.6 }}>
                    ⚠️ <strong>Keep all records indefinitely?</strong><br />
                    Therapy records can accumulate years of detailed health-adjacent data with no automatic cleanup.
                  </div>
                  <button onClick={() => setES(prev => ({ ...prev, retentionDays: 0 }))}
                    style={{ padding: "8px 14px", borderRadius: 9, border: "none", background: "#C07070",
                      color: "#fff", fontWeight: 700, cursor: "pointer", fontSize: 13, fontFamily: "inherit" }}>
                    Yes, keep all records
                  </button>
                </div>
              </details>
            )}
