 ...
 * Generated by bundle.js on 2026-05-27 02:53 UTC
 *
 ...
 *   ProfileModule.jsx
 *   lib/healthCheck.js
 *   ContentLibraryModule.jsx
 *   FamilyModule.jsx
 *   data/adaptiveQuiz.js
 *   NamingModule.jsx
...
const TOOLS = [
  ...
  { id: "memory",           label: "Memory Practice",   icon: "💭",  desc: "Recall facts about loved ones" },
  ...
  { id: "progress",         label: "Progress",           icon: "📈",  desc: "Track sessions & skill trends" },
  { id: "family",           label: "Family & Friends",   icon: "👨‍👩‍👧‍👦",  desc: "People & facts for exercises" },
  { id: "content",          label: "Photo Library",      icon: "🖼️",  desc: "Personal photos for exercises" },
  { id: "profile",          label: "Profile",            icon: "👤",  desc: "Patient name & condition" },
];
...
const APP_KEY_PREFIXES = ["ppa_", "fam_"];
...
const SYSTEM_PROMPT = `You are Dr. Aria, a compassionate and expert Speech-Language Pathologist (SLP) with broad clinical experience across a wide range of speech and language disorders — including aphasia (post-stroke, progressive, and acquired), traumatic brain injury (TBI), dementia-related language decline, and other conditions affecting communication.
...
const API_KEY_STORAGE_KEY = "ppa_api_key";
...
const CAREGIVER_PIN_KEY     = "ppa_caregiver_pin";
const DEFAULT_CAREGIVER_PIN = "0000";
...
async function checkCaregiverPin(pin) {
...
async function setCaregiverPin(pin) {
  localStorage.setItem(CAREGIVER_PIN_KEY, await hashPin(pin));
}

async function isCaregiverPinDefault() {
  const stored = localStorage.getItem(CAREGIVER_PIN_KEY);
  if (!stored) return true;
  return checkCaregiverPin(DEFAULT_CAREGIVER_PIN);
}

function CaregiverPinEntry({ onSuccess, onCancel }) {
...
function ChangeCaregiverPinForm({ onClose }) {
...
const DEFAULT_CONDITION = "primary_progressive";

function bktInitialState() {
  return Object.fromEntries(Object.entries(BKT_PARAMS).map(([s, p]) => [s, p.p_known0]));
}

function applyTimeDecay(knowledge, elapsedDays, skill, conditionType) {
...
function bktUpdate(prior, correct, hintUsed, skill, conditionType = DEFAULT_CONDITION) {
...
function bktApplySession(bktState, attempts, conditionType, lastSessionMs, nowMs = Date.now()) {
  let next = { ...bktState };
...
function bktSnapshotFromSession(bktState, nowMs = Date.now()) {
...
function bktTrajectory(snapshots, conditionType = DEFAULT_CONDITION) {
  const profile = CONDITION_PROFILES[conditionType] ?? CONDITION_PROFILES[DEFAULT_CONDITION];
...
function bktAllTrajectories(skillSnapshots, conditionType = DEFAULT_CONDITION) {
  return Object.fromEntries(
...
function logEntryToAttempt(entry) {
...
function ProgressModule({ sessionLog, bkt, bktSnapshots, conditionType }) {
...
async function probeUrl(url, mediaType = "image", ms = PROBE_TIMEOUT_MS) {
...
function itemHealthStatus(item) {
...
async function runLaunchHealthCheck(items, onProgress, signal) {
...
async function runRemoteHealthCheck(items, signal) {
  const now = Date.now();
...
function applyHealthResults(items, updates) {
...
const CONTENT_KEY   = "fam_content_items";
const CONTENT_TAGS  = ["person", "family", "animal", "pet", "place", "food", "object", "plant", "vehicle", "other"];
...
function ContentLibraryModule({ items, onUpdate }) {
...
function saveFamilyMembers({ persons, relationships }) {
  try {
...
// ── Main module ────────────────────────────────────────────────────────────────
function FamilyModule({ members, onUpdate }) {
...
function aqRecord({ factKey, category, correct, hintLevel = 0, wrongOption = null }) {
  const aq = load();
  const now = Date.now();
...
function aqGetConfidence(factKey, category, conditionType = "primary_progressive") {
  const aq = load();
  const now = Date.now();
...
function aqGetDifficultyTier(factKey, category, conditionType) {
...
function aqMostConfused(factKey) {
...
function aqSaveDistractorTags(clipId, questionTags) {
  const aq = load();
  aq.distractorTags ??= {};
...
function aqGetDistractorTags(clipId) {
  return load().distractorTags?.[clipId] ?? null;
}

function aqNeedsTagging(clipId) {
...
function SentenceModule({ addToLog, conditionType = DEFAULT_CONDITION }) {
  // ── data ────────────────────────────────────────────────────────────────────
...
function AdaptiveChoiceQuestion({
  question,
  factKey,
  category,
  conditionType = "primary_progressive",
...
// ---- VIDEO MODULE ----
function VideoModule({ addToLog, conditionType = "primary_progressive" }) {
  // ── mode ─────────────────────────────────────────────────────────────────────
...
const MEMORY_QUALITY_KEY = "fam_memory_quality";
...
function MemoryModule({ members, addToLog }) {
...
function App() {
  ...
  // Purge progress entries older than the configured retention window
  useEffect(() => { ppaRunRetentionPurge(); }, []); // eslint-disable-line react-hooks/exhaustive-deps
